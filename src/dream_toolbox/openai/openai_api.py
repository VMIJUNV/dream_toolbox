...
class LLMV2:
    ...
    def query(self,prompt,model_args = None,**kwargs):
        if model_args is None:
            model_args = self.default_model_args

        if self.use_cache:
            input={'prompt':prompt,'model_args':model_args,'kwargs':kwargs}
            inp_encoded = self.cache.input_encode(input)
            response = self.cache.find_cache(inp_encoded)
            if response is not None:
                return response
        ...
        cached_tokens_ = getattr(completion.usage, 'prompt_tokens_details', None)
        cached_tokens = getattr(cached_tokens_, "cached_tokens", None)

        response['usage_prompt_tokens'] = prompt_tokens
        response['usage_completion_tokens'] = completion_tokens
        response['usage_total_tokens'] = total_tokens
        response['usage_cached_tokens'] = cached_tokens
        response['usage_prompt_tokens'] = prompt_tokens
...
class EmbeddingV2:
    ...
    def generate(self,text,model_args = None,**kwargs):
        ...
        try:
            ...
            response['status']=1
            response['embedding'] = [item.embedding for item in embedding.data]
        except Exception as e:
            response['status']=0
